...
func TestAssignCatalog(t *testing.T) {
	...
	node, err = f.Apply(ctx, a, plan.NewShowProcessList(), nil, DefaultRuleSelector)
	require.NoError(err)
	...
	node, err = f.Apply(ctx, a, plan.NewShowDatabases(), nil, DefaultRuleSelector)
	require.NoError(err)
	sd, ok := node.(*plan.ShowDatabases)
	require.True(ok)
	require.Equal(a.Catalog, sd.Catalog)

	node, err = f.Apply(ctx, a, plan.NewLockTables(nil), nil, DefaultRuleSelector)
	require.NoError(err)
	lt, ok := node.(*plan.LockTables)
	require.True(ok)
	require.Equal(a.Catalog, lt.Catalog)

	node, err = f.Apply(ctx, a, plan.NewUnlockTables(), nil, DefaultRuleSelector)
	require.NoError(err)
